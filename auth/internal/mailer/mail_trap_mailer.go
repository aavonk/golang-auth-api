...
// our email templates. This has a comment dirrection in the format `//go:embed "<path>"`
// IMMEDIATELY ABOVE it, which indicates to Go that we want to store the contents of the
// ./templates directory in the templateFS embedded fyle system.
...
func New(host string, port int, username, password, sender string) MailTrapMailer {
...
func (m MailTrapMailer) Send(recipient, templateFile string, data interface{}) error {
	...
	// Try sending the email up to three times before aborting and returning the
	// final error. We sleep for 500 milliseconds between each attempt
	for i := 1; i < 3; i++ {
